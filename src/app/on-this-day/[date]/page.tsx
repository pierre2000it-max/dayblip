...
/** Parse "january-1", "july-4" → { month: 1-12, day: 1-31 } */
...
/** Get prev / next date slug, correctly crossing month/year boundaries */
function adjacentSlug(month: number, day: number, delta: 1 | -1): string {
  // Use 2024 (leap year) so Feb-28 +1 = Feb-29 is valid, and overflow wraps correctly
  const d = new Date(2024, month - 1, day + delta);
  return `${MONTH_SLUG[d.getMonth()]}-${d.getDate()}`;
}

/** Format a slug as display label: "january-1" → "January 1st" */
...
export async function generateMetadata({
  ...
  const label = parsed
    ? `${MONTH_DISPLAY[parsed.month - 1]} ${ordinal(parsed.day)}`
    : params.date;
  return {
    title: `On This Day: ${label} — History, Events & Birthdays`,
    description: `Discover what happened on ${label} in history. Famous birthdays, major events and historical facts.`,
  };
}
...
export default function OnThisDayPage({
  ...
  // ── Invalid slug (not a recognisable date format) ─────────────────────
  if (!parsed) {
    return (
                ...
                {[...data.events]
                  ...
                  .map((ev) => (
                    ...
                    >
                      ...
                      {/* Event */}
                      <p className="flex-1 pt-0.5 leading-relaxed text-white">
                        {ev.event}
                      </p>
                    </div>
                  ))}
            ...
            <div className="mx-auto max-w-[900px]">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                {data.birthdays.map((b) => (
                  <div
                    key={b.name}
                    className="rounded-xl border border-[#0f3460] bg-[#16213e] p-5"
                  >
        ...
        <section className="bg-[#16213e] px-6 py-16">
          <div className="mx-auto max-w-[900px] text-center">
            ...
            <p className="mb-8 text-[#a8a8b3]">
              ...
              <strong className="text-white">{formattedDate}</strong>. Check
              back soon!
            </p>
            <p className="mb-6 text-[#a8a8b3]">
              In the meantime, explore one of these popular dates:
            </p>
